...
// Where the page file is written on disk. Always <path>.html, IN PLACE — clean
// URLs are served extensionless by .htaccess, so no directory restructuring (and
// therefore no MultiViews conflict / broken relative links).
function outFileFor(relPath) {
  return noExtOf(relPath) + ".html";
}
// Public URL (clean = extensionless). index -> "", dir/index -> "dir/", foo -> "foo".
function cleanFromNoExt(n) {
  if (n === "index" || n === "") return "";
  if (n.endsWith("/index")) return n.slice(0, -"index".length); // "dir/index" -> "dir/"
  return n; // "python/02-routing" (extensionless; .htaccess maps it to .html)
}
...
export function build(config, { quiet = false } = {}) {
  ...
  for (const page of pages) {
    ...
    const dest = join(config.outPath, outFileFor(page.relPath));
  ...
  // 6) clean-URL .htaccess (Apache): serve extensionless, 301 old .html, no
  // MultiViews (which is what 500'd the directory-style attempt). Files stay
  // in place as <path>.html, so relative links keep working.
  if (clean) writeFileSync(join(config.outPath, ".htaccess"), htaccessBody(base));
...
function htaccessBody(base) {
  const b = (base || "/").replace(/\/*$/, "/"); // ensure single trailing slash
  return `# Generated by tina4press - extensionless clean URLs.
Options -MultiViews
DirectoryIndex index.html
RewriteEngine On
RewriteBase ${b}

# Canonicalize: 301 any direct .html request to its clean URL.
RewriteCond %{THE_REQUEST} \\s/+(.*/)?index\\.html[\\s?] [NC]
RewriteRule ^ ${b}%1 [R=301,L]
RewriteCond %{THE_REQUEST} \\s/+(.+?)\\.html[\\s?] [NC]
RewriteRule ^ ${b}%1 [R=301,L]

# Serve an extensionless request from its .html file (no redirect).
